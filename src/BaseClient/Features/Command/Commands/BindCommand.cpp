...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	auto modOpt = modules.getByName(moduleName);
	if (!modOpt.has_value()) {
	...
	if (args->size() >= 3) {
		...
		if (key.size() > 1) {
			...
			if (key == "none") {
				mod->setKeybind(0x0);
				clientMessageF("%sSuccessfully unbound %s!", GREEN, mod->getName());
				return true;
			}
			...
			for (int i = 0; i < 190; i++) {
				...
				if (strcmp(needle, haystackLowercase) == 0) {
					mod->setKeybind(i);
					clientMessageF("%sThe keybind of %s is now '%s'", GREEN, mod->getName(), haystack);
		...
		if (keyCode >= 0x30 && keyCode <= 0x5A) {
			...
			if (!modOpt.has_value()) {
				...
				clientMessageF("%sThe Keybind of %s is now '%c'", GREEN, mod->getName(), keyCode);
			}
		}
		else {
		...
		clientMessageF("%s%s is currently bound to %s", RED, mod->getName(), Utils::getKeybindName(mod->getKeybind()));
		return true;
	}
